...
BOOST_AUTO_TEST_CASE(socket_test) {      printf("Running socket_test...\n");          string aszXmlData = "{\"jsonrpc\":\"2.0\",\"method\":\"eth_subscribe\",\"params\":[\"newHeads\",{}], \"id\": 1337}";      string aszXmlRequest;      string aszRequest;      string aszHostIp="127.0.0.1";      string aszPort="8545";           aszRequest = strprintf("http://%s:%s HTTP/1.1",aszHostIp,aszPort);      printf("Method and Resource path is below:\n\n\n");      printf("%s",aszRequest.c_str());      aszHostIp += ":" + aszPort;       printf("\n\nHOST header is below:\n\n\n");     printf("%s",aszHostIp.c_str());      aszXmlRequest = strprintf("POST %s\r\nHost: %s\r\nContent-Type: application/json\r\nContent-Length: %d\r\n\r\n%s\r\n",aszRequest,aszHostIp,aszXmlData.size(),aszXmlData);      printf("\n\nPOST Request which send to the server:\n\n\n");     printf("%s",aszXmlRequest.c_str());     int fd = 0;     struct sockaddr_in demoserverAddr;      fd = socket(AF_INET, SOCK_STREAM, 0);      if (fd < 0)     {         printf("Error : Could not create socket\n");           }     else     {         demoserverAddr.sin_family = AF_INET;         demoserverAddr.sin_port = htons(8545);         demoserverAddr.sin_addr.s_addr = inet_addr("127.0.0.1");         memset(demoserverAddr.sin_zero, '\0', sizeof(demoserverAddr.sin_zero));     }      if (connect(fd, (const struct sockaddr *)&demoserverAddr, sizeof(demoserverAddr)) < 0)     {          printf("ERROR connecting to server\n");              }      int total = aszXmlRequest.size();     int sent = 0;     do {         int bytes = write(fd,aszXmlRequest.c_str()+sent,total-sent);         if (bytes < 0)             error("ERROR writing message to socket");         if (bytes == 0)             break;         sent+=bytes;     } while (sent < total);      printf("Written data\n");     while(1){         char buffer[256];        int n = read(fd,buffer,256);         if (n < 0)               error("ERROR reading from socket");              if(n> 0)             printf("%s\n",buffer);             MilliSleep(100);       } } 
