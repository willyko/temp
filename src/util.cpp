...
bool parseEthMethodInputData(const std::vector<unsigned char>& vchInputExpectedMethodHash, std::vector<unsigned char>& vchInputData, CAmount& outputAmount) {
    // input not 50 bytes is bad
    if(vchInputData.size() != 50) 
        return false;

    // method hash is 8 bytes starting in pos 2
    std::vector<unsigned char>::const_iterator first = vchInputData.begin() + 2;
    std::vector<unsigned char>::const_iterator last = vchInputData.begin() + 10;
    ...
    // get the encoded method params - there are two params - 
    // uint256 and uint, which is 32 + 8 = 40 bytes so this string
    // must contain exactly 40 bytes or it's not what we're expecting
    first = vchInputData.begin() + 10;
    last = vchInputData.end();
    const std::vector<unsigned char> vchEncodedParams(first, last);

    // This check is not strictly necessary but I like it
    if(vchEncodedParams.size() != 40) 
        return false;

    // get the first parameter and convert to CAmount and assign to output var
    first = vchEncodedParams.begin();
    last = vchEncodedParams.begin() + 32;
    const std::vector<unsigned char> vchParam1Amount(first, last);

    // convert the vch into a uint64_t (CAmount)
    // outputAmount = 
    //       (vchParam1Amount[0] << 56) 
    //     | (vchParam1Amount[1] << 48) 
    //     | (vchParam1Amount[2] << 40) 
    //     | (vchParam1Amount[3] << 32) 
    //     | (vchParam1Amount[4] << 24) 
    //     | (vchParam1Amount[5] << 16) 
    //     | (vchParam1Amount[6] << 8) 
    //     | vchParam1Amount[7];

    outputAmount = * reinterpret_cast<const int64_t *>(&vchParam1Amount);

    // get the second parameter and convert to CAmount and assign to output var
    // commented out for now since it's unused but I wanted it  here for clarity
    // and potential afuture enhancements
    // first = vchEncodedParams.begin() + 32;
    // last = vchEncodedParams.end();
    // const vector<unsigned char> vchParam2Amount(first, last);
